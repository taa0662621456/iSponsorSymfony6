<?php
	...
	class CheckRequirementsSubscriber implements EventSubscriberInterface
	{
		...
		#[ArrayShape([ConsoleEvents::ERROR => "string", KernelEvents::EXCEPTION => "string"])] public static function getSubscribedEvents(): array
		{
			return [
		...
		public function handleConsoleError(ConsoleErrorEvent $event): void
		{
			...
			if ($event->getCommand() && \in_array($event->getCommand()->getName(), $commandNames, true)) {
				if ($this->isSQLitePlatform() && !\extension_loaded('sqlite3')) {
					...
					$io->error(
						'This command requires to have the "sqlite3" PHP extension enabled because, by default, the Symfony Demo application uses SQLite to store its information.'
					);
				}
			}
		}
		...
		public function handleKernelException(ExceptionEvent $event): void
		{
			...
			if ($isDriverException && $this->isSQLitePlatform() && !\extension_loaded('sqlite3')) {
				$event->setException(
					new \Exception(
						'PHP extension "sqlite3" must be enabled because, by default, the Symfony Demo application uses SQLite to store its information.'
					)
				);
			}
		}
		...
		private function isSQLitePlatform(): bool
		{
			...
			return $databasePlatform && 'sqlite' === $databasePlatform->getName();
		}
	}